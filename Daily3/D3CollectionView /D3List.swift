...
public class D3List : UIView {
    ...
    var refreshControl: UIRefreshControl?{
        didSet{
            self.d3ListView.refreshControl = refreshControl
        }
    }
...
extension D3List: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if (dataSource?.numberOfItems(inD3List: collectionView as! D3ListView)) != nil{
            return (dataSource?.numberOfItems(inD3List: collectionView as! D3ListView))!}
        return 0
    }
...
extension D3List: UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if let customSize = delegate?.sizeForItem?(d3CollectionView: d3ListView, index: indexPath.row) {
            ...
            cellHeight = customSize.height - (customSize.height - xInsets > 0 ? yInsets : 0)
        }else{
...
extension D3List{
    ...
    public func numberOfItems(inSection section: Int) -> Int{
       //   numberOfItems = dataSource?.numberOfItems(inD3List: self.d3ListView) ?? 0
        print("Main: \(self.d3ListView.numberOfItems(inSection: section))")
    ...
    public func reloadSections(_ sections: IndexSet){
        self.d3ListView.reloadSections(sections)
    }
